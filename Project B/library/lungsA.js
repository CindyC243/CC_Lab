...
let maxSpotSize = 50;
let backButton;
let particleCount = 0; // Track the count of particles drawn
let lungTint = 150; // Initial lung tint

let breathDirection = 1;
let breathSpeed = 0.003;
    ...
    if (dist(mouseX, mouseY, lungX, lungY) < lungImg.width * lungScale / 2) {
      ...
      if (lungImg.pixels[index + 3] > 0) {
        ...
        if (!currentSpot) {
          ...
          if (particleCount >= 70) {
            lungTint = 50; // Change lung tint after 70 particles
          }
        }
      }
    }
